...
# Handle broken pipe errors when using head/tail
def signal_handler(signum, frame):
    sys.exit(0)

signal.signal(signal.SIGPIPE, signal_handler)

# Store original print before replacement
_original_print = print

# Safe print function to handle BrokenPipeError
...
# Replace built-in print with safe_print
print = safe_print
...
class PerformanceBenchmark:
    ...
    def test_simulation_speed(self, duration: int = 300) -> Dict:
        ...
        safe_print(f"初始资源: CPU {initial_resources['cpu_percent']:.1f}%, RAM {initial_resources['memory_mb']:.1f}MB")
        ...
        safe_print(f"✅ 仿真完成:")
        safe_print(f"   仿真时间: {sim_time_elapsed:.1f}s")
        safe_print(f"   实际时间: {real_time_used:.2f}s")
        safe_print(f"   速度比率: {speed_ratio:.1f}x 实时")
        safe_print(f"   内存使用: {final_resources['memory_mb']:.1f}MB")
        safe_print(f"   生成订单: {results['orders_generated']}")
        safe_print(f"   发生故障: {results['faults_occurred']}")
    ...
    def test_scalability(self) -> List[Dict]:
        ...
        for duration in test_durations:
            ...
            if len(scalability_results) > 1:
                ...
                safe_print(f"   性能变化: {speed_change:+.1f}% (相比上一级别)")
    ...
    def test_memory_usage(self) -> Dict:
        ...
        for target_time in test_points:
            ...
            safe_print(f"时间 {target_time:3d}s: 内存 {resources['memory_mb']:6.1f}MB | "
        ...
        safe_print(f"\n📊 内存分析:")
        safe_print(f"   初始内存: {initial_memory:.1f}MB")
        safe_print(f"   最终内存: {final_memory:.1f}MB")
        safe_print(f"   内存增长: {memory_growth:.1f}MB")
        safe_print(f"   增长率: {memory_growth/initial_memory*100:.1f}%")
        
        if memory_growth / initial_memory > 0.5:
            safe_print("   ⚠️ 内存增长较快，可能存在内存泄漏")
        else:
            safe_print("   ✅ 内存使用合理")
    ...
    def test_event_processing_rate(self) -> Dict:
        ...
        safe_print(f"✅ 事件处理分析:")
        safe_print(f"   测试时长: {real_time:.2f}s")
        safe_print(f"   总事件数: {total_events}")
        safe_print(f"   处理速率: {events_per_second:.1f} 事件/秒")
        safe_print(f"   订单事件: {total_orders}")
        safe_print(f"   状态更新: {device_status_events}")
        safe_print(f"   故障报警: {fault_alert_events}")
    ...
    def test_state_space_performance(self) -> Dict:
        ...
        results = {
            ...
            'total_transitions': state_stats['state_transitions'],
        ...
        safe_print(f"✅ 状态空间分析:")
        safe_print(f"   观察到的唯一状态: {results['unique_states']:,}")
        safe_print(f"   状态转换次数: {results['total_transitions']:,}")
        safe_print(f"   状态生成速率: {states_per_second:.1f} 状态/秒")
        safe_print(f"   转换速率: {transitions_per_second:.1f} 转换/秒")
        safe_print(f"   理论状态空间: {results['state_space_size']}")
        
        coverage_percentage = results['unique_states'] / results['state_space_size'] * 100
        safe_print(f"   状态空间覆盖: {coverage_percentage:.8f}%")
        
        return results
    
    def run_complete_benchmark(self) -> Dict:
        """运行完整的性能基准测试"""
        safe_print("🚀 SUPCON 系统性能基准测试")
        safe_print("=" * 80)
        safe_print("这将测试系统在不同负载下的性能表现...")
        safe_print()
        
        benchmark_results = {}
        
        # 1. 基础仿真速度测试
        safe_print("【第1项】基础仿真速度测试")
    ...
    def generate_performance_report(self, results: Dict):
        ...
        safe_print(f"🎯 核心性能指标:")
        safe_print(f"   仿真速度: {base_speed:.1f}x 实时")
        safe_print(f"   内存使用: {base_memory:.1f}MB")
        safe_print(f"   事件处理: {results['event_processing']['events_per_second']:.1f} 事件/秒")
        ...
        safe_print(f"\n📈 可扩展性评估:")
        safe_print(f"   平均性能变化: {avg_speed_change:+.1f}% (每级别)")
        if avg_speed_change > -5:
            safe_print(f"   评级: 优秀 ✅")
        elif avg_speed_change > -15:
            safe_print(f"   评级: 良好 🟡")
        else:
            safe_print(f"   评级: 需要优化 🔴")
        
        # 内存效率
        memory_growth_rate = results['memory_usage']['growth_rate']
        safe_print(f"\n🧠 内存效率:")
        safe_print(f"   内存增长率: {memory_growth_rate*100:.1f}%")
        ...
        safe_print(f"\n🌐 状态空间处理:")
        safe_print(f"   状态生成速率: {states_per_sec:.1f} 状态/秒")
        safe_print(f"   状态转换效率: {state_stats['total_transitions']:,} 转换")
        ...
        safe_print(f"\n🏆 综合性能评分:")
        safe_print(f"   仿真速度: {speed_score:.1f}/100")
        safe_print(f"   内存效率: {memory_score:.1f}/100")
        safe_print(f"   事件处理: {event_score:.1f}/100")
        safe_print(f"   总体得分: {overall_score:.1f}/100")
        
        if overall_score >= 80:
            safe_print(f"   系统评级: 优秀 🎉")
        elif overall_score >= 60:
            safe_print(f"   系统评级: 良好 👍")
        else:
            safe_print(f"   系统评级: 需要优化 💪")
...
def main():
    """主程序"""
    safe_print("⚡ SUPCON 系统性能基准测试")
    safe_print("这将全面测试系统的性能表现和资源使用情况")
    safe_print("测试可能需要几分钟时间...")
    safe_print()
