...
| **Conveyor_3**   | 传送带 C-Q | (65, 20)  | 长度:10m, 速度:0.5m/s, 容量:3 个产品 |
...
| AGV_ID    | 操作区域 | 初始位置 | 移动速度 | 载重     | 电池容量         |
| --------- | -------- | -------- | -------- | -------- | ---------------- |
| **AGV_1** | **下方** | (10, 15) | 2.0 m/s  | 5 个产品 | 100%, 充电 30 秒 |
| **AGV_2** | **上方** | (10, 25) | 2.0 m/s  | 5 个产品 | 100%, 充电 30 秒 |

**AGV 路径点**:
为实现分区运行和精确交互，路径点被重新设计为工站的**前后交互点**，并严格区分 AGV 的上下轨道。

- **交互点**:

  - `A_pre`: (12, 20) / `A_post`: (18, 20)
  - `B_pre`: (32, 20) / `B_post`: (38, 20)
  - `C_pre`: (52, 20) / `C_post`: (58, 20)
  - `Q_pre`: (72, 20) / `Q_post`: (78, 20)
  - `RawMaterial`: (5, 20) / `Warehouse`: (85, 20)

- **AGV 专用轨道点**:
  - **下方轨道**: AGV_1 在 y=15 的轨道上移动，通过垂直移动到达 y=20 的交互点。
  - **上方轨道**: AGV_2 在 y=25 的轨道上移动，通过垂直移动到达 y=20 的交互点。
  - **充电站**: `Charge_1` (10, 10), `Charge_2` (10, 30)

### 2.2 搬运规则（分区作业）

#### 传送带职责（自动搬运）

- **Conveyor_1**: 工站 A → 工站 B
- **Conveyor_2**: 工站 B → 工站 C
- **Conveyor_3**: 工站 C → 质检站
- **触发条件**: 工站完成处理，传送带有空位，下游有容量。
...
1.  **原料配送**: 原料仓库 → 工站 A
2.  **成品输出**: 质检站 → 成品仓库
3.  **返工搬运 (下游)**: 质检站 → 工站 C / 工站 B

**AGV_2 (上方区域)**:

1.  **特殊工艺路径 (P3)**: 从 `C_post` 取货，运至 `B_pre`。
2.  **返工搬运 (上游)**: 从 `Q_post` 取货，运至 `A_pre`。
3.  **应急搬运**: 替代故障传送带。例如，替代`Conveyor_1`时，从`A_post`取货，运至`B_pre`。

### 2.3 产品类型和精确工艺路径

基于智能制造流程设计，每个工站执行特定的制造工艺：

- **工站 A（初级加工）**：基础组装、元器件处理、初步加工
- **工站 B（精密加工）**：精密处理、连接加工、质量控制
- **工站 C（综合测试）**：功能测试、性能验证、参数调整
- **质检站**：最终检验、品质确认、出厂准备
...
1. **标准化流程（P1/P2）**：
   ...
   - 采用传送带自动流转，减少 AGV 调度复杂度
   - 工站间无返工，流水线式生产
   - 适合大批量标准化生产

2. **复杂定制流程（P3）**：

   - 需要**二次加工**：部分产品需要返回特定工站进行额外处理。
   - **AGV 关键调度**：工站 C 到工站 B 的返回需要 AGV 精确调度。
   - **暂存缓冲区**：工站 C 配备特殊缓冲区，支持 P3 产品暂存。
   - **更长测试时间**：复杂产品需要更全面的测试验证。

3. **实际制造约束**：
   - **设备预热时间**：焊接工站需要 15-30 分钟预热（游戏中简化）
   - **换线时间**：不同产品类型切换需要 5-10 分钟调整（影响调度策略）
   - **质量批次效应**：连续生产同类产品质量更稳定
   - **工装治具限制**：P3 产品需要专用工装，数量有限
...
- **对选手隐藏**: 在具体某次故障事件中，`根本原因`是未知的，Agent 必须通过`症状`和历史数据进行推断。

#### 故障症状与诊断手册 (示例)

| 症状 (Symptom) - _Agent 可观察_ | 可能的根本原因 (Root Cause) - _Agent 需诊断_                                          | 维修指令 (Action) - _Agent 需发出_                                                                       | 基准维修时间                 | 错误诊断惩罚   |
| :------------------------------ | :------------------------------------------------------------------------------------ | :------------------------------------------------------------------------------------------------------- | :--------------------------- | :------------- |
...
| `"AGV路径阻塞"`                 | 1. **临时障碍物**: 其他 AGV 或人员占用路径点。<br>2. **定位模块失灵**: 自身硬件故障。 | 1. `reroute_agv(target, 'new_path')` 或 等待<br>2. `reboot_device(target)`                               | 自动恢复(30-120s)<br>`60s`   | 任务超时       |
...
| 约束类型         | 描述                                                              | 示例                                                       |
| ---------------- | ----------------------------------------------------------------- | ---------------------------------------------------------- |
| **能源成本**     | 不同时间段或工站处理不同产品时，能源消耗和成本不同。              | 高峰时段电价是平时的 1.5 倍；StationC 能耗较高。           |
| **材料兼容性**   | 某些工站可能无法处理特定材料，或处理时效率/次品率更高。           | StationB 无法处理 P3 产品的特殊合金，强制执行会 100%失败。 |
| **维护资源限制** | 维修团队或备件在同一时间是有限的（例如，同时只能维修 1 台设备）。 | Agent 需要对维修任务进行排程。                             |
| **供应链波动**   | 原材料供应可能随机中断，Agent 需管理库存或寻找替代方案。          | 组件 A 缺货，持续 120-300 秒。                             |
...
2. **诊断准确率**: `(正确诊断次数 / 总故障数) × 100%`
3. **平均故障恢复时间**: `Σ(故障解决时间 - 故障发生时间) / 总故障数`
4. **AGV 利用率**: `(AGV运输时间 / (总时间 - 故障时间 - 充电时间)) × 100%`
...
- **状态同步**: 每 100ms 更新一次状态，通过 MQTT 发布。仿真环境与选手 Agent 之间通过结构化的 JSON 消息进行通信，我们鼓励选手在自己的 Agent 内部构建自然语言处理能力，形成 `JSON -> NL -> JSON` 的完整决策链路。
...
| `factory/resource/{id}/status` | **Subscribe** | 订阅所有 AGV 的状态                           | JSON (结构化)      |
| `factory/orders/new`           | **Subscribe** | 接收新订单信息                                | JSON (结构化)      |
| `factory/kpi/update`           | **Subscribe** | 订阅 KPI 更新                                 | JSON (结构化)      |
| `factory/agent/commands`       | **Publish**   | **核心输出**: 发布选手 Agent 生成的结构化指令 | JSON (结构化)      |
...
| :-------------------- | :------------------------ | :------ | :---------------------------------------- |
| `request_maintenance` | 请求对设备进行维护        | 设备 ID | `{"maintenance_type": "replace_bearing"}` |
| `move_agv`            | 命令 AGV 移动到指定路径点 | AGV ID  | `{"destination_id": "P9"}`                |
| `reroute_order`       | 重新规划订单路径          | 订单 ID | `{"target_station_id": "StationC"}`       |
| `adjust_priority`     | 调整订单优先级            | 订单 ID | `{"priority": "high"}`                    |
| `emergency_stop`      | 紧急停止某个设备          | 设备 ID | `{}`                                      |
...
- 评委和管理员可通过 UI 界面，选择设备和故障类型，实时注入故障，测试 Agent 的响应能力。
...
**生产效率得分 (50%)**:
`效率得分 = (订单完成率 × 0.5 + 1 / 平均生产周期 × 0.5) × 50` (与基线对比归一化)

**鲁棒性得分 (50%)**:
`鲁棒性分 = (诊断准确率 × 0.6 + 1 / 平均恢复时间 × 0.4) × 50`

### 7.2 测试场景

- **标准测试 (2h)**: 正常生产 → 设备故障 → AGV 故障 → 原料短缺。
- **压力测试 (30min)**: 高频订单 + 多重并发故障。
- **现场演示 (15min)**: 评委实时注入**手册之外**的未知故障，考验 Agent 的泛化能力。
